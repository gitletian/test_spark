	...
	* */
	def main(args: Array[String]): Unit = {
		...
		val conf = new SparkConf()
			...
			.set("spark.shuffle.sort.bypassMergeThrshold", "256") // 测试默认值 200 ?
		...
		tpa_people_angle.sort(expr("type_code").asc, expr("value").desc).collect().foreach(row => data_func(row))
